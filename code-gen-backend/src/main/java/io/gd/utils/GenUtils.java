...
import io.gd.dto.ComponentConfigDTO;
import io.gd.dto.GenPageInfoDTO;
import io.gd.entity.ColumnEntity;
import io.gd.entity.TableEntity;
import io.gd.entity.mongo.MongoDefinition;
import io.gd.entity.mongo.MongoGeneratorEntity;
...
public class GenUtils {
    ...
    public static List<String> getTemplates() {
        ...
        templates.add("template/ListReqDTO.java.vm");
        templates.add("template/ListResDTO.java.vm");
        templates.add("template/SaveReqDTO.java.vm");
        templates.add("template/SaveReqConverter.java.vm");
    ...
    public static void generatorCode(Map<String, String> table,
                                     List<Map<String, String>> columns, GenPageInfoDTO genPageInfo, ZipOutputStream zip) {
        //配置信息
        ...
        //设置vue2-admin页面需要的组件属性
        //columsList.stream().filter()
        //Map<String, Object> componentConfig = new HashMap<>();//存到 Velocity 中的组件配置
        Map<String, List<ComponentConfigDTO>> componentConfig = genPageInfo.getComponentConfig();
        for (Map.Entry<String, List<ComponentConfigDTO>> entry : componentConfig.entrySet()) {
            System.out.println("key = " + entry.getKey() + ", value = " + entry.getValue());
            columsList.forEach(el -> {
                //List entryValue2 = Arrays.asList(entry.getValue());
        ...
        prop.put("file.resource.loader.path", "org.apache.velocity.runtime.resource.loader.ClasspathResourceLoader");
        Velocity.init(prop);
        String mainPath = config.getString("mainPath");
        mainPath = StringUtils.isBlank(mainPath) ? "io.gd" : mainPath;
        //封装模板数据
        ...
        //
        map.put("componentConfig", componentConfig);
        ...
        for (String template : templates) {
            ...
            try {
                //添加到zip
                zip.putNextEntry(new ZipEntry(getFileName(template, tableEntity.getClassName(), config.getString("package"), config.getString("moduleName"), (String)map.get("pathName"))));
     ...
     * 获取文件名
     */
    public static String getFileName(String template, String className, String packageName, String moduleName, String pathName) {
        String packagePath = "main" + File.separator + "java" + File.separator;
        if (StringUtils.isNotBlank(packageName)) {
            packagePath += packageName.replace(".", File.separator) + File.separator + moduleName + File.separator + pathName + File.separator;
        }
        ...
        if (template.contains("index.js.vm")) {
            return "main" + File.separator + "resources" + File.separator + "src" + File.separator + "api" + File.separator + moduleName + File.separator + className.toLowerCase() + ".js";
        }

        if (template.contains("ListReqDTO.java.vm")) {
            return packagePath + "dto" + File.separator + className + "ListReqDTO.java";
        }

        if (template.contains("ListResDTO.java.vm")) {
            return packagePath + "dto" + File.separator + className + "ListResDTO.java";
        }
