...
import { getDatabaseTableList } from "@/api/code-gen/database-table-selector.js";
...
export default {
  ...
  data() {
    return {
       ...
       * 表格 相关数据开始
       * */
      tableCommonAttr: { "row-key": "table-name" },
      tableColMetaData: [
        {
          colType: "selection",
          key: "multi-selection",
        ...
        { type: "Text", key: "table-name", label: "表名" },
        { type: "Text", key: "engine", label: "引擎" },
        { type: "Text", key: "table-comment", label: "表备注" },
        { type: "Text", key: "create-time", label: "创建时间" },
        {
        ...
        componentMetaDataArr: [
          {
            ...
            curRowAttr: (row, rowIdx) => {
              let attrObj = {};
              if (row["table-name"] === this.curPickedTableName) {
                attrObj.textVal = "已选中";
            ...
            events: (rowIdx, colIdx, colKey, ...data) => {
              return {
                click: () => {
                  console.log(rowIdx, colIdx, colKey, data);
                  this.curPickedTableName = this.tableBodyData[rowIdx][
                    "table-name"
                  ];
                  this.$emit(
                    "initFormDataAboveTable",
                    this.tableBodyData[rowIdx]["table-name"]
                  );
                }
              };
            },
