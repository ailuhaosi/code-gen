...
/**列出所有目前内建的表单item组件类型 */
const allBuildInFormItemTypes = ["Input", "Select", "CheckboxGroup", "RadioGroup", "Switch", "DatePicker", "TimePicker"];// CustomizeFormItem


// ////////////////
export const FilterFormTableModal = {
  data () {
    return {
      ...
      footerBlockComponentsModal: [{
        ...
        events: (eventFromKey) => {
          return {
            click: () => {
              this.mixins_emitEventStrategy("DialogForm", "FormCancelThenClearDialogVisible", eventFromKey);
            }
          }
        },
        isShow: true
        ...
        events: (...args) => {
          return {
            click: () => {
              this.mixins_emitEventStrategy("DialogForm", "ValidSubmit", ...args);
            }
          }
        }
      }],
  ...
  computed: {
    ...
    formAboveTableCanToggledShowStatusKeys () {
      return this.formItemMetaListAboveTable.filter((itemMeta, idx) => idx >= this.formAboveTableCanToggledShowStatusOverIdx).map(itemMeta => itemMeta.key);
    },
    /** 表格上方的表单布局样式 自动根据表单右边按钮数量宽度变化; */
    mixins_formAboveTableLayout () {
      const isLayoutRatioWider = this.footerBlockComponentsAboveTable.filter(el => ((el?.isShow ?? true) && (el.operationBtnType !== "multi-select-operation-btn"))).length >= 3
    ...
    mixins_SaveEditDialogFormItemMetaListAlias () {
      return `${this.mixins_firstLowerCaseBusinessKey}SaveDialogFormItemMetaList`;
    },
    mixins_firstUpperCaseBusinessKey () {
      const businessKey = this.mixins_firstLowerCaseBusinessKey;
      const getFirstUpperCaseBusinessKey = businessKey.replace(businessKey[0], businessKey[0].toUpperCase());
  ...
  methods: {
    ...
    mixins_toggledShowFormAboveTable (eventFromKey) {
      ...
      // 切换当前按钮样式;即 展开V  <--->   收缩^
      const eventFromKeyIdx =
        this.footerBlockComponentsAboveTable.findIndex(
     ...
     */
    // 快捷的重置表单 统一方法
    mixins_speedFormReset (formDataAlias = 'formDataAboveTable', specialInitMap = {}) {
      const resData = {}
     ...
     * */
    mixins_sameEventStrategyNameMergeLogic (localStrategys, appendStrategys, buildInStrategyKeys) {
      ...
      appendStrategyKeys.forEach(newKey => {
        ...
        const canAppendStrategy = (oldStrategyKeys.includes(newKey) && !buildInStrategyKeys.includes(newKey)) || !oldStrategyKeys.includes(newKey);
        canAppendStrategy ? localStrategys[newKey] = appendStrategys[newKey] : null;
      })
    },
    /**
     * 组件的元数据的key名相同时的合并逻辑
     * 合并元数据的策略,并直接修改 localStrategys 为合并结果
     ...
     * */
    mixins_sameComponentMetaKeyNameMergeLogic (localComponentMetaList, appendComponentMetaList) {
      if (Array.isArray(appendComponentMetaList?.componentKeysArr)) {
        let results = [];
        appendComponentMetaList?.componentKeysArr.forEach((appendKey, idx) => {
          const curComponent = localComponentMetaList.find(component => component.key === appendKey);
     ...
     */
    mixins_doDialogTipOperationStrategy (strategyName, pickedRowIdx = null, args) {
      ...
      const strategys = {
        DeleteRowDialogTipOpen: async () => {
          ...
          const res = await (this.getBussinessCurdMethodName("Delete"))([deleteId]);
        ...
        TableBatchDeleteDialogTipOpen: async () => {
          ...
          const res = await (this.getBussinessCurdMethodName("Delete"))(deleteIds);
          const isSuccessReq = res.code === 0;
      ...
      //用户自定义的事件策略
      const appendStrategys = this.getDialogTipOpenEventStrategy(strategyName, pickedRowIdx, args);

      //合并事件策略
      this.mixins_sameEventStrategyNameMergeLogic(strategys, appendStrategys, buildInStrategyKeys);
     ...
     * @param {int | null} pickedRowIdx 来自表格触发中的行索引;null说明来自其他按钮触发
     *  */
    async mixins_doInitDialogFormOperationStrategy (strategyName, pickedRowIdx = null, args) {
      const buildInStrategyKeys = [];//strategys中内置不能被外部覆盖的策略
      const strategys = {
      ...
      //用户自定义的事件策略
      const appendStrategys = this.getDialogFormOpenEventStrategy(strategyName, pickedRowIdx, args);
    ...
    mixins_doFormAboveTableEventStrategy (receivedArgs) {
      ...
      //用户自定义的事件策略
      const appendStrategys = this.getFormCustomizeEventStrategyAboveTable(strategyName, args);

      //合并事件策略
      this.mixins_sameEventStrategyNameMergeLogic(strategys, appendStrategys, buildInStrategyKeys);

      // 弹窗相关策略校验提示
      if (strategyName.startsWith('TableBatch') && this.curPickedTableRowsData.length === 0) {
        this.$message.warning('至少选择一项')
        return
      } else if (strategyName.startsWith('TableSingle') && this.curPickedTableRowsData.length !== 1) {
      ...
      if (strategyName.endsWith("DialogFormOpen")) {
        ((innerArgs) => {
          this.mixins_doInitDialogFormOperationStrategy(strategyName, null, innerArgs);
        })(args);
      } else if (strategyName.endsWith("DialogTipOpen")) {
        ((innerArgs) => {
          this.mixins_doDialogTipOperationStrategy(strategyName, null, innerArgs);
        })(args);
      } else {
    ...
    mixins_doTableEventStrategy (receivedArgs) {
      ...
      const appendStrategys = this.getTableCustomizeEventStrategy(strategyName, pickedRowIdx, restArgs);
      ...
      if (strategyName.endsWith("DialogFormOpen")) {
        ((innerArgs) => {
          ...
          this.mixins_doInitDialogFormOperationStrategy(strategyName, pickedRowIdx, restArgs);
        })(args);
      } else if (strategyName.endsWith("DialogTipOpen")) {
        ((innerArgs) => {
          const [pickedRowIdx, ...restArgs] = innerArgs;
          this.mixins_doDialogTipOperationStrategy(strategyName, pickedRowIdx, restArgs);
        })(args);
    ...
    mixins_doDialogFormEventStrategy (receivedArgs) {
      ...
      const buildInStrategyKeys = ["FormCancelThenClearDialogVisible", "ValidSubmit"];//strategys中内置不能被外部覆盖的策略
      ...
      const strategys = {
        ...
        ValidSubmit: async eventFromKey => {
          ...
          const appendStrategys = this.getDialogFormValidSubmitEventStrategy(this.curDialogInfo.eventFrom, this.curDialogInfo.pickedRowIdx, this.curDialogInfo.args);//TODO:FIXME: 这里参数不能是 strategyName
          ...
          this.mixins_emitEventStrategy("DialogForm", "ThenClearDialogVisible", eventFromKey);
        }
      };

      //用户自定义的事件策略
      const appendStrategys = this.getDialogFormCustomizeEventStrategy(strategyName, args);
    ...
    mixins_emitEventStrategy (emitTarget, eventStrategyName, ...args) {
      ...
      const emitTo = {
        "FormAboveTable": () => {
          this.$refs["FormAboveTableRef"].doOperation(
            eventStrategyName,
            ...args
          );
        },
        "Table": () => {
          this.$refs["TableRef"].doOperation(
            eventStrategyName,
            ...args
          );
        },
        "DialogForm": () => {
          this.$refs["DialogFormRef"].doOperation(
            eventStrategyName,
            ...args
          );
        }
    ...
    mixins_toggleShowDialogFooterBtn (isShow, keyName = 'form-cancel') {
      const dialogFooterBtnIdx = this.footerBlockComponentsModal.findIndex(el => el.key === keyName)
      if (dialogFooterBtnIdx !== -1) {
        this.$set(this.footerBlockComponentsModal[dialogFooterBtnIdx], 'isShow', isShow)
      }
    },
    /**
     * 弹窗表单时需要获取info数据
    ...
    mixins_initFormDataAboveTable () {
      ...
      this.formItemMetaListAboveTable.forEach(itemMeta => {
        ...
        formDataAboveTable[itemMeta.key] = this?.initFormDataAboveTable?.[itemMeta.key] ?? undefined;
      })
    ...
    mixins_initFormAboveTableCanToggledShowStatusKeys () {
      if (this.formAboveTableCanToggledShowStatusKeys.length > 0) {
        const moreBtnIdx = this.footerBlockComponentsAboveTable.findIndex(el => el.key === "form-more-above-table" && (el?.isShow ?? true));
        moreBtnIdx > -1 ? this.$set(this.footerBlockComponentsAboveTable[moreBtnIdx], "isShow", true) : null;
        this.formAboveTableCanToggledShowStatusKeys.forEach(toggledKey => {
          const canToggledKeyIdx = this.formItemMetaListAboveTable.findIndex(el => el.key === toggledKey);
          if (canToggledKeyIdx > -1) {
            this.$set(this.formItemMetaListAboveTable[canToggledKeyIdx], "isShow", false)
          }
        })
        return;
      } else {
        const moreBtnIdx = this.footerBlockComponentsAboveTable.findIndex(el => el.key === "form-more-above-table" && (el?.isShow ?? true));
        moreBtnIdx > -1 ? this.$set(this.footerBlockComponentsAboveTable[moreBtnIdx], "isShow", false) : null;
      }
    },
    /** 合并 初始化表格上方表单的所有按钮组件 */
    mixins_initMergeFooterBlockComponentsAboveTable (emitTarget = "FormAboveTable") {
      //内建的组件元数据
      const localComponentMetaList = [
        ...
        },
        {
          ...
          events: eventFromKey => {
            return {
              click: () => {
                this.mixins_emitEventStrategy(emitTarget, "FormReset", eventFromKey);
              }
            };
          }
        },
        {
          ...
          events: eventFromKey => {
            return {
              click: () => {
                this.mixins_emitEventStrategy(emitTarget, "FormSearch", eventFromKey);
              }
            };
          },
          isShow: checkBtnPermission(this, this.getBussinessBtnPerms("Search"))
        },
        {
          ...
          events: eventFromKey => {
            return {
              click: async () => {
                this.mixins_emitEventStrategy(emitTarget, "FormAddDialogFormOpen", eventFromKey);
              }
            };
          },
        ...
        },
        {
          ...
          events: eventFromKey => {
            return {
              click: async () => {
                this.mixins_emitEventStrategy(emitTarget, "TableBatchDeleteDialogTipOpen", eventFromKey);
              }
            };
          },
          isShow: checkBtnPermission(this, this.getBussinessBtnPerms("Delete"))
        }
      ];
      // 用户追加的自定义的组件元数据
      const appendComponentMetaList = this?.getAppendFooterBlockComponentsAboveTable?.(emitTarget) ?? null;
      // 合并后返回 并赋值
      this.footerBlockComponentsAboveTable = this.mixins_sameComponentMetaKeyNameMergeLogic(localComponentMetaList, appendComponentMetaList);
    },
    // 合并 初始化表格内的所有按钮组件
    mixins_initMergedTableOperationsBlockComponents (emitTarget = "Table") {
      ...
      // 用户追加的自定义的组件元数据
      const appendComponentMetaList = this?.getAppendTableOperationsBlockComponents?.(emitTarget) ?? null;
      // 找到表格的"操作按钮块"索引
      const tableOperationsBlockIdx = this.tableColMetaData.findIndex(colMetaData => colMetaData.key === "table-operations-block");
      // 合并后返回
      const mergedComponentList = this.mixins_sameComponentMetaKeyNameMergeLogic(localComponentMetaList, appendComponentMetaList);
      // 赋值合并结果
      this.$set(this.tableColMetaData[tableOperationsBlockIdx], "componentList", mergedComponentList);
    },
